...
const resolvePrimaryStartConfig = Effect.fn("desktop.backendConfiguration.resolvePrimary")(
  ...
  > {
    ...
    return {
      ...
      // entryPath is payload-aware (resolveActiveBackendEntryPath) so a staged
      // JS payload is applied on backend (re)start; it falls back to the
      // shell-bundled backend entry when no payload is active.
      args: [entryPath, "--bootstrap-fd", "3"],
      entryPath,
...
export const make = Effect.gen(function* () {
  ...
  const buildWindowsPrimaryConfig = Effect.gen(function* () {
    ...
    return yield* resolvePrimaryStartConfig({ ...shared, resourceMonitorPath }).pipe(
      ...
      // resolveActiveBackendEntryPath (payload-aware entry resolution) reads
      // the staged-payload pointer from disk, so the primary start config
      // needs FileSystem too.
      Effect.provideService(FileSystem.FileSystem, fileSystem),
    );
  });
