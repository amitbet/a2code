...
export interface TimelineEntriesProjection {
  ...
  readonly userInputExchanges: ReadonlyArray<UserInputExchange>;
...
function extractToolDetail(
  ...
  // Command rows: the block above already prefers real output whenever the
  // provider reports the command text. Upstream stops there and leaves the
  // detail empty; the fork keeps going, because Cursor reports neither the
  // command nor a title and the row would otherwise render blank. Prefer the
  // output, then fall back to a non-zero exit summary — without it a failed
  // command is indistinguishable from a successful one, here and in the
  // failure heuristic.
  if (commandTool || isCommandToolDetail(payload, heading)) {
    const output = extractToolOutput(payload);
...
function timelineEntryFromUserInput(exchange: UserInputExchange): TimelineEntry {
  return {
    id: exchange.id,
    kind: "user-input",
...
export function deriveTimelineEntriesWithState(
  ...
  if (
    ...
    previous.userInputExchanges.length === userInputExchanges.length &&
    hasExactArrayPrefix(previous.proposedPlans, proposedPlans) &&
    hasExactArrayPrefix(previous.workEntries, workEntries) &&
    hasExactArrayPrefix(previous.userInputExchanges, userInputExchanges)
  ) {
    const entries = replaceStreamingTimelineMessages(messages, previous);
    if (entries !== null)
      return { messages, proposedPlans, workEntries, userInputExchanges, entries };
  }
  const canAppend =
    previous !== null &&
  ...
  if (canAppend) {
    ...
    const userInputRows = userInputExchanges
      .slice(previous.userInputExchanges.length)
      .map(timelineEntryFromUserInput);
    const suffix = [...messageRows, ...proposedPlanRows, ...workRows, ...userInputRows].toSorted(
  ...
  const userInputRows = userInputExchanges.map(timelineEntryFromUserInput);
  return {
    messages,
    proposedPlans,
    workEntries,
    userInputExchanges,
    entries: [...messageRows, ...proposedPlanRows, ...workRows, ...userInputRows].toSorted(
      compareTimelineEntriesByCreatedAt,
    ),
  };
}
