...
describe("branding", () => {
  it("uses injected desktop branding when available", async () => {
    ...
    expect(branding.APP_BASE_NAME).toBe("A2 Code");
    expect(branding.APP_STAGE_LABEL).toBe("Nightly");
    expect(branding.APP_DISPLAY_NAME).toBe("A2 Code (Nightly)");
  });
  ...
  it("does not label the latest hosted app channel", async () => {
    ...
    expect(branding.APP_DISPLAY_NAME).toBe("A2 Code");
  });
