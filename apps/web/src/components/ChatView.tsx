...
function ChatViewContent(props: ChatViewProps) {
  ...
  const onSend = async (
    ...
    // The upload channel is image-only (its mint contract accepts image MIME
    // types alone), so non-image attachments keep riding the inline data-URL
    // path. Split once here and reuse the split everywhere below.
    const uploadableComposerImages = composerImagesSnapshot.filter(
      (image) => image.type === "image",
    );
    if (supportsAttachmentUploads && uploadableComposerImages.length > 0) {
      for (const image of uploadableComposerImages) {
        startAttachmentUpload({ environmentId, image });
      }
      await awaitAttachmentUploads(uploadableComposerImages.map((image) => image.id));
      if (getUploadedAttachments({ environmentId, images: uploadableComposerImages }) === null) {
      ...
      if (shouldQueuePrompt) {
        ...
        if (startResult._tag === "Failure") {
          ...
          if (supportsAttachmentUploads) {
            releaseAttachmentUploads(uploadableComposerImages);
          }
