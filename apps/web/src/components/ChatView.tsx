...
function ChatViewContent(props: ChatViewProps) {
  ...
  // Full server-side error: the local override wins over the persistent
  // session error.
  ...
  const logicalProjectEnvironments = useMemo(() => {
    ...
    const memberProjects = allProjects.filter(
      (candidate) =>
        deriveLogicalProjectKeyFromSettings(candidate, projectGroupingSettings) === logicalKey,
    );
  ...
  useEffect(() => {
    ...
    const lastVisitedAt = routeThreadLastVisitedAt ? Date.parse(routeThreadLastVisitedAt) : NaN;
  ...
  const timelineEntries = useMemo(
    ...
    [activeThread?.proposedPlans, timelineMessages, userInputExchanges, workLogEntries],
  );
  ...
  const onEditQueuedPrompt = useCallback(
    async (prompt: (typeof queuedPromptsWithPreviewUrls)[number]) => {
      ...
      let restoredFileAttachments: ComposerFileAttachment[];
      try {
        ...
        // Images and generic files ride separate composer slots, so the
        // restored attachments split by kind before going back into the draft.
        restoredImages = restoredFiles.flatMap(({ attachment, file }) =>
          isImageAttachment(attachment)
            ? [{ ...attachment, previewUrl: URL.createObjectURL(file), file }]
            : [],
        );
        restoredFileAttachments = restoredFiles.flatMap(({ attachment, file }) =>
          isFileAttachment(attachment) ? [{ ...attachment, file }] : [],
        );
      } catch (error) {
        toastManager.add(
          stackedThreadToast({
      ...
      addComposerDraftFiles(composerDraftTarget, restoredFileAttachments);
  ...
  }, [cancelVideoPreviewRequest, resetLocalDispatch, routeThreadKey]);
  ...
  const onSend = async (
    ...
    if (shouldQueuePrompt) {
      ...
      if (shouldAnchorFirstMessage) {
        // The new row becomes the anchored end-space target so it lands near
        // the top while the response streams into the reserved space below it.
        isAtEndRef.current = true;
                            ...
                            <div className="mx-auto mb-2 w-full max-w-208 rounded-lg border border-border/70 bg-card/95 p-2 shadow-sm">
                              ...
                              <div className="flex flex-col gap-1.5">
                                {queuedPromptsWithPreviewUrls.map((prompt) => {
                                  const images = prompt.attachments.filter(isImageAttachment);
                                  return (
                                    <div
                                        ...
                                        <div className="flex shrink-0 -space-x-1">
                                          {images.slice(0, 3).map((image) => (
                                            <Tooltip key={image.id}>
                                              <TooltipTrigger
                                                render={
                                                  <div className="size-7 overflow-hidden rounded border border-border bg-muted">
                                                    {image.previewUrl ? (
                                                      <img
                                              ...
                                              />
                                              <TooltipPopup side="top">{image.name}</TooltipPopup>
                                            </Tooltip>
                                          ))}
                                          {images.length > 3 ? (
