...
const ATTACHMENT_SIZE_LIMIT_LABEL = `${Math.round(
  PROVIDER_SEND_TURN_MAX_ATTACHMENT_BYTES / (1024 * 1024),
)}MB`;
...
export const ChatComposer = memo(function ChatComposer(props: ChatComposerProps) {
  ...
  const composerMenuItems = useMemo<ComposerCommandItem[]>(() => {
    ...
    if (composerTrigger.kind === "slash-command") {
      ...
      // Forking copies the existing conversation context, so it only applies to
      // a started server thread — not a draft that has no context yet.
      const forkSlashCommandItem = {
        id: "slash:fork",
      ...
      if (item.type === "slash-command") {
        ...
        if (item.command === "fork") {
          ...
          if (typeof composerDraftTarget !== "string") {
            void forkThread(composerDraftTarget);
          }
          return;
        }
  ...
  const addComposerImages = async (files: File[]) => {
    ...
    for (const file of files) {
      ...
      const isImage = isImageFile(file) || isHeicImage;
      // Any file type may be attached. Images still have to be a MIME type the
      // providers accept; non-images are only rejected on size, since they are
      // sent byte-for-byte instead of being downscaled to fit.
      ...
      if (!isImage && file.size > PROVIDER_SEND_TURN_MAX_ATTACHMENT_BYTES) {
    ...
    try {
      ...
      for (const file of acceptedFiles) {
        // Non-image attachments carry no preview and skip compression: they are
        // sent byte-for-byte and were already size-checked above.
        if (!isImageFile(file) && !isHeicImageFile(file)) {
          nextImages.push({
            type: "file",
            id: randomUUID(),
                    ...
                    {composerImages
                      ...
                      .map((image) => {
                        ...
                        return (
                          <div
                            key={image.id}
                            className="relative flex min-h-16 min-w-16 max-w-48 items-center overflow-hidden rounded-lg border border-border/80 bg-background"
                          >
                            {image.type === "image" && image.previewUrl ? (
                              ...
                              <div className="flex min-h-16 items-center px-2 py-1 text-left text-[10px] text-secondary-label">
