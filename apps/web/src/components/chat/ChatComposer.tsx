...
export const ChatComposer = memo(function ChatComposer(props: ChatComposerProps) {
  ...
  const addComposerImages = async (files: File[]) => {
    ...
    for (const file of files) {
      // Non-images are rejected on size; oversized images are downscaled to
      // fit in the build phase below rather than refused.
      if (!isImageFile(file) && file.size > PROVIDER_SEND_TURN_MAX_ATTACHMENT_BYTES) {
                    ...
                    .map((image) => (
                      ...
                      >
                        {image.type === "image" && image.previewUrl ? (
                          ...
                          <div className="flex min-h-16 items-center px-2 py-1 text-left text-[10px] text-muted-foreground/70">
