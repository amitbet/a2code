...
export const MessagesTimeline = memo(function MessagesTimeline({
  ...
  const setTimelineHostElement = useCallback((element: HTMLDivElement | null) => {
  ...
  const alwaysRender = useMemo(() => {
    ...
    // Pins are keyed, so a restore target expressed as an index cannot be
    // carried alongside them; the selection is the live concern either way.
    const baseKeys = base && "keys" in base ? base.keys : [];
    return { keys: [...baseKeys, ...selectionPinnedRowKeys] };
...
function buildToolCallExpandedBody(
  ...
  // Second pass: upstream's `seen` dedupes exact text, this one collapses
  // blocks that differ only by tool-label formatting.
...
const PlainWorkEntryRow = memo(function PlainWorkEntryRow(props: {
  ...
  workspaceRoot: string | undefined;
  isExpandedToolGroupEntry: boolean;
  // In-chat find (fork) mounts the collapsed tool body while a match is in it.
  searchExpanded?: boolean;
  displayLabel?: string | undefined;
  onToggleEntry?: ((collapsed: boolean) => void) | undefined;
}) {
  ...
  // In-chat find forces a collapsed row open; gate the body on the same flag or
  // a search-forced row expands to nothing.
  const expandedBody = effectiveExpanded
    ...
    >
      ...
      {workEntry.generatedImage ? (
        <button
          ...
          onClick={(event) => {
            event.stopPropagation();
            const generatedImage = workEntry.generatedImage;
            if (!generatedImage) return;
            onImageExpand({
              images: [
                {
                  name: generatedImage.alt ?? "Generated image",
      ...
      {effectiveExpanded && viewedImage && threadRef ? (
        <div
      ...
      {effectiveExpanded && workEntry.questionAnswer ? (
        <QuestionAnswerHistory answer={workEntry.questionAnswer} />
      ) : null}
      {effectiveExpanded && canExpand && expandedBody && !workEntry.questionAnswer ? (
        <div
