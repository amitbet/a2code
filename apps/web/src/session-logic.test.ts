...
describe("deriveWorkLogEntries", () => {
  ...
  it("surfaces a non-zero exit code when the provider omits the command text", () => {
    ...
    const [entry] = deriveWorkLogEntries(activities);
    // The command's own output is the detail when there is one; the row still
    // reads as failed because the text says so.
    expect(entry?.detail).toBe("cursor: command not found");
    expect(workEntryIndicatesToolFailure(entry!)).toBe(true);
  });

  it("falls back to the exit code when a failed command produced no output", () => {
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "cmd-failed-silent",
        kind: "tool.completed",
        summary: "Ran command",
        tone: "tool",
        payload: {
          ...
          data: {
            kind: "execute",
            rawInput: {},
            rawOutput: { exitCode: 127, stdout: "", stderr: "" },
          },
        },
      }),
    ];
  ...
  it("shows a successful command's output as its detail", () => {
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "cmd-ok",
        kind: "tool.completed",
    ...
    const [entry] = deriveWorkLogEntries(activities);
    expect(entry?.detail).toBe("all good");
  });
  ...
  it("uses command stdout as the detail when Cursor omits the command input", () => {
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
