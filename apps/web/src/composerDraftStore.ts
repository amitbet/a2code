...
export function deriveEffectiveComposerModelState(input: {
  ...
  // The legacy key only applies when no instance is selected, which already
  // covers upstream's narrower antigravity-only guard against a custom
  // instance inheriting it.
...
export function hydrateImagesFromPersisted(
  ...
  return attachments.flatMap((attachment) => {
    // Drafts persisted by older fork builds could park a non-image here (files
    // used to share the image slot). Drop those instead of hydrating a chip
    // that would render as a broken image; files now persist separately.
      ...
      return {
        ...
        clearComposerPromptAndImages: (threadRef) => {
          ...
          set((state) => {
            ...
            for (const image of current.images) {
              // Non-image attachments carry no preview URL.
              if (image.previewUrl) {
                revokeObjectPreviewUrl(image.previewUrl);
              }
            }
