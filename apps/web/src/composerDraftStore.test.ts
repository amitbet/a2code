...
describe("composerDraftStore syncPersistedAttachments", () => {
  ...
  it("drops legacy non-image persisted attachments instead of hydrating broken images", () => {
    const persistApi = useComposerDraftStore.persist as unknown as {
      getOptions: () => {
        merge: (
          persistedState: unknown,
    ...
    // Older fork builds parked generic files in the image slot. Files now
    // persist separately, so a stale non-image entry is dropped rather than
    // hydrated as an image the composer cannot render.
    expect(mergedState.draftsByThreadKey[threadKeyFor(threadId)]?.images).toEqual([]);
  });
});
