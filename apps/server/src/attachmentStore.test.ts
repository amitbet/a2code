...
describe("attachmentStore", () => {
  ...
  it("resolves non-image attachment paths by id", () => {
    const attachmentsDir = NodeFS.mkdtempSync(
      NodePath.join(NodeOS.tmpdir(), "t3code-attachment-store-"),
    );
    try {
      // Non-image extensions are not probed by fallback, so the id has to carry
      // the extension (that is what createAttachmentId(threadId, ext) mints).
      const attachmentId = createAttachmentId("thread-1", ".pdf")!;
