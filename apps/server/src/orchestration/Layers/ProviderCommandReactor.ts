 ...
 */
export class ThreadReferenceUnresolvedError extends Schema.TaggedError<ThreadReferenceUnresolvedError>()(
...
const make = Effect.gen(function* () {
  ...
  const buildSendTurnRequestForThread = Effect.fnUntraced(function* (input: {
    ...
    // Read as thread facts, not the thread detail: a turn start must never
    // decode older message bodies (one unreadable row would block every turn).
    const forkContext = yield* projectionSnapshotQuery
      .getThreadForkContext(input.threadId)
      .pipe(Effect.map(Option.getOrUndefined));
    ...
    const requestedReferences: ReadonlyArray<RequestedThreadReference> = [
      ...(isFirstUserMessageTurn && forkedFromId !== undefined
        ...
        : []),
      ...parsedReferences.local.map((reference): RequestedThreadReference => ({
        kind: "local",
    ...
    for (const entry of orderedReferences.slice(0, MAX_THREAD_REFERENCES)) {
      ...
      const referenced = yield* resolveThreadDetail(ThreadId.make(entry.threadId));
      if (!referenced) {
  ...
  const processSessionSet = Effect.fn("processSessionSet")(function* (
    ...
    const prompt = (yield* projectionSnapshotQuery.getThreadQueuedPrompts(
      event.payload.threadId,
    ))[0];
    if (!prompt) {
      return;
