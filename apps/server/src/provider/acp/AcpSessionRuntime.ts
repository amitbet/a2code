            ...
            promptSerializationSemaphore.withPermit(
              Effect.gen(function* () {
                ...
                if (yield* isCancelled) {
                  // A caller forked on `dispatched` must not park forever just
                  // because the prompt was cancelled before it reached the wire.
                ...
                // Closes the window between forking the RPC and publishing the
                // fiber, where a cancel would have found nothing to interrupt.
                if (yield* isCancelled) {
                  yield* Fiber.interrupt(promptRpcFiber).pipe(Effect.ignore);
                }
                // The prompt is now the registered active prompt, so a later
                // `cancel` targets it; release any caller waiting on this.
                if (promptOptions?.dispatched) {
                  yield* Deferred.succeed(promptOptions.dispatched, undefined);
                }
      ...
      cancel: getStartedState.pipe(
        Effect.flatMap((started) =>
          Effect.gen(function* () {
            ...
            // semaphore or not yet on the wire.
