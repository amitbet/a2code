...
describe("ClaudeAdapterLive", () => {
  ...
  it.effect("puts the command text last so attachments do not suppress expansion", () => {
    ...
    return Effect.gen(function* () {
      ...
      // The fork ingests PDFs as document blocks (see attachmentContent.ts);
      // the command text still has to be the last block.
      assert.deepEqual(prompts[2]?.message.content, [
        {
          type: "document",
          source: {
            type: "base64",
  ...
  it.effect("completes with result usage without querying current context usage", () => {
    ...
    return Effect.gen(function* () {
      ...
      if (usageEvent?.type === "thread.token-usage.updated") {
        // Fork: `maxTokens` only rides context-accurate readings. This usage is
        // an accumulated request total (input+output summed across API calls),
        // which overcounts the live window, so the quota meter must not pair it
        // with the context window. See `contextAccurate` in ClaudeAdapter.
  ...
  it.effect("inlines non-image file attachments as text in Claude user messages", () => {
    const baseDir = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "claude-attachments-file-"));
    const harness = makeHarness({
    ...
    return Effect.gen(function* () {
      ...
      const attachmentPath = NodePath.join(attachmentsDir, attachmentRelativePath(attachment)!);
      NodeFS.mkdirSync(NodePath.dirname(attachmentPath), { recursive: true });
      NodeFS.writeFileSync(attachmentPath, new TextEncoder().encode('{"a": 1}'));
      ...
      const promptMessage = yield* Effect.promise(() => readFirstPromptMessage(createInput));
      assert.isDefined(promptMessage);
      // The inlined file precedes the prompt so the user's text stays the last
      // block, where Claude reads slash commands from.
      assert.deepEqual(promptMessage?.message.content, [
        {
          type: "text",
          text: [
  ...
  it.effect("includes the selected Claude context window on context-accurate task usage", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      ...
      yield* adapter.sendTurn({
        threadId: THREAD_ID,
        input: "hello",
        // The synthetic catalog's "expanded" window is 1M, keeping this
        // independent of the bundled/remote manifest.
        modelSelection: createModelSelection(
          ProviderInstanceId.make("claudeAgent"),
          SYNTHETIC_CLAUDE_CAPABLE_MODEL,
          [{ id: "contextWindow", value: "expanded" }],
      ...
      harness.query.emit({
        type: "system",
        subtype: "task_progress",
        task_id: "task-usage-context-window",
        description: "Thinking through the patch",
        usage: {
          used_tokens: 91_000,
          total_tokens: 250_000,
        },
        session_id: "sdk-session-task-usage-context-window",
        uuid: "task-usage-progress-context-window",
      } as unknown as SDKMessage);
  ...
  it.effect("does not include maxTokens when direct Claude used tokens are zero", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      ...
      harness.query.emit({
        ...
        result: "done",
        stop_reason: "end_turn",
        session_id: "sdk-session-result-usage-zero-direct",
        usage: {
          used_tokens: 0,
          total_tokens: 535000,
        },
