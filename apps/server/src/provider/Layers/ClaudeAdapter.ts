...
const buildUserMessageEffect = Effect.fn("buildUserMessageEffect")(function* (
  ...
  for (const attachment of input.attachments ?? []) {
    // The attachment union is open, so a newer server can send a type this
    // build cannot classify. Those reach the agent through the path line
    // ProviderService puts in the prompt instead.
