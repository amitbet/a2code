...
export function HomeScreen(props: HomeScreenProps) {
  ...
  // The v1 grouped Home rows expose a single Pin/Unpin toggle; the v2 rows use
  // upstream's explicit pin/unpin menu entries.
  const handleTogglePinThread = useCallback(
    (thread: EnvironmentThreadShell) => {
      void (thread.pinnedAt == null ? props.onPinThread(thread) : props.onUnpinThread(thread));
    },
    [props.onPinThread, props.onUnpinThread],
  );
