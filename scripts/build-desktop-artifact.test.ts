...
// @effect-diagnostics nodeBuiltinImport:off - Tests use Node's glob matcher to verify electron-builder exclusions, and packaged-archive fixtures compute the sidecar digest with the same Node primitive as the builder.
...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it.effect("defaults desktop updater metadata to the A2 Code release repository", () =>
    Effect.gen(function* () {
  ...
  it.effect("lets explicit desktop updater metadata override the fork default", () =>
    Effect.gen(function* () {
  ...
  it.effect("applies platform-specific packaging to the build config", () =>
    Effect.gen(function* () {
      ...
      // of unpacking thousands of loose files at install time. Unpacking all of
      // node_modules previously made macOS ad-hoc signing walk tens of thousands
      // of files and exhaust the open-file limit (EMFILE).
      ...
      assert.deepStrictEqual((linux.linux as Record<string, unknown>).protocols, [
        { name: "A2 Code", schemes: ["t3code", "t3code-dev"] },
      ]);
  ...
  it.effect("adds passkey entitlements and both renderer protocols to signed macOS builds", () =>
    Effect.gen(function* () {
      ...
      assert.equal(config.appId, "com.amitbet.a2code");
