...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it.effect("applies platform-specific packaging to the build config", () =>
    Effect.gen(function* () {
      ...
      // Only the Windows WSL backend needs an unpacked tree: it launches plain
      // `wsl.exe -- node`, which cannot read inside an asar. mac/linux must stay
      // packed — unpacking all of node_modules made macOS ad-hoc signing walk
      // tens of thousands of files and exhaust the open-file limit (EMFILE).
