...
export const ServerProviderUsageWindow = Schema.Struct({
  ...
  /**
   * Fork addition: a provider-supplied figure the percentage alone cannot
   * carry, rendered beside the label. Today only Claude's extra-usage window
   * sets it, with the spend it represents (`$169.27 / $1,000.00`).
   */
...
export const ServerProviderUsageLimits = Schema.Struct({
  ...
  /** Fork addition: the plan the windows belong to (`pro`, `max`), when the provider names one. */
