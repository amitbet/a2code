...
const makeHarness = Effect.fn("TestThreadCommands.makeHarness")(function* () {
  ...
  const runtime = Atom.runtime(
    Layer.mergeAll(
      ...
      // The fork's thread commands (fork/copy-reference) reach for these two;
      // nothing under test here calls them.
      Layer.succeed(
        ThreadSnapshotLoader,
        ThreadSnapshotLoader.of({ load: () => Effect.die("unused") }),
      ),
      remoteHttpClientLayer(() => Promise.reject(new Error("unused"))),
    ),
  );
